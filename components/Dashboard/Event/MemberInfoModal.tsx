            ...
            <section onClick={e => e.stopPropagation()} className="modal_container">
                ...
                <div className="p-5 grid grid-cols-2 gap-4">
                    <div className="col-span-2 flex flex-row items-center gap-x-2">
                        ...
                        <span className={`user_avatar_${member.scheme}_text text-base`}>{member.name}</span>
                    </div>

                    <div className="col-span-1 flex flex-col gap-y-2">
                        <span className="text-sm text-gray-500 dark:text-gray-400">مجموع هزینه کرد ها:</span>
                        <span className="text-base text-gray-700 dark:text-gray-300">{TomanPriceFormatter(getAllPersonExpends(member.id).toString())} تومان</span>
                    </div>
                    <div className="col-span-1 flex flex-col gap-y-2">
                        <span className="text-sm text-gray-500 dark:text-gray-400">مجموع بدهی ها:</span>
                        <span className="text-base text-gray-700 dark:text-gray-300">{TomanPriceFormatter(getAllPersonDebts(member.id).toString())} تومان</span>
                    </div>
                    <div className="col-span-1 flex flex-col gap-y-2">
                        <span className="text-sm text-gray-500 dark:text-gray-400">مجموع پول های دریافتی:</span>
                        <span className="text-base text-gray-700 dark:text-gray-300">{TomanPriceFormatter(getAllPersonRecieved(member.id).toString())} تومان</span>
                    </div>
                    <div className="col-span-1 flex flex-col gap-y-2">
                        <span className="text-sm text-gray-500 dark:text-gray-400">مجموع پول های ارسالی:</span>
                        <span className="text-base text-gray-700 dark:text-gray-300">{TomanPriceFormatter(getAllPersonSent(member.id).toString())} تومان</span>
                    </div>
                    <div className="col-span-1 flex flex-col gap-y-2">
                        <span className="text-sm text-gray-500 dark:text-gray-400">وضعیت تسویه:</span>
                        <span className="text-base text-gray-700 dark:text-gray-300">{getPersonBalanceStatus(member.id)}</span>
                    </div>
                    <div className="col-span-1 flex flex-col gap-y-2">
                        <span className="text-sm text-gray-500 dark:text-gray-400">تسویه:</span>
                        <span className="text-base text-gray-700 dark:text-gray-300">{TomanPriceFormatter(Math.abs(getPersonBalance(member.id)).toFixed(0))} تومان</span>
                    </div>
